...
#define SWIPE_ST 0.3
#define SWIPE_MIN 60.0
#define SWIPE_MAX 1000.0
...
La(matrix<T> &L, std::vector<T> &f, std::vector<T> &fERBs,
    std::vector<std::complex<double>> &fo, int w2, int hi, int i)
{
	size_t j;
	std::vector<T> a(w2);
    ...
    const std::vector<T> &x, std::vector<T> &fERBs, T nyquist, int w, int w2)
{
    ...
    std::vector<std::complex<double>> fi(w);
    std::vector<std::complex<double>> fo(w);
#pragma omp critical
    fft_plan(w);
	std::vector<T> hann(w);
...
#pragma omp crtical
{
	...
	for (i = 1; i < L.size() - 2; i++) {
		for (j = 0; j < (size_t)w; j++)
			fi[j] = {(double)(x[j + offset] * hann[j]), 0.};
#pragma omp critical
{
        for (j = 0; j < (size_t)w; j++)
            fft_in(w)[j] = fi[j];
...
template <typename T>
static T
swipe(const std::vector<T> &x, int samplerate)
{
	size_t i;
	T td = 0.;
...
void Pitch::estimate_SWIPE(const std::vector<ArrayXd> & frames, double fs, ArrayXd & pitch)
{
    pitch.setZero(frames.size());

#pragma omp parallel for default(none) shared(std::cout, frames, fs, pitch) schedule(dynamic)
    for (int i = 0; i < frames.size(); ++i) {
        const ArrayXd & x = frames.at(i);
        std::vector<double> v(x.data(), x.data() + x.size());

        double f0 = swipe(v, fs);

#pragma omp critical
        pitch(i) = f0;
#pragma omp critical
        std::cout << "Pitch frame #" << (i + 1) << ": " << round(pitch(i)) << " Hz" << std::endl;   
    }       
}
